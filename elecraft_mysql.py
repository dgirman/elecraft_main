...
class MySqlMain:
    ...
    def make_new_radio_tables(self):
        ...
        cursor.execute(
            "CREATE TABLE presets (preset_name VARCHAR(100) , preset_comment VARCHAR(300), mykey INTEGER(100) NOT NULL AUTO_INCREMENT PRIMARY KEY, mydatetimestamp VARCHAR(255) DEFAULT CURRENT_TIMESTAMP,platform VARCHAR(255), "
        ...
        cursor.execute(
            "CREATE TABLE settings (pointer VARCHAR(1) DEFAULT '1', mykey INTEGER(100) NOT NULL AUTO_INCREMENT PRIMARY KEY, mydatetimestamp VARCHAR(255) DEFAULT CURRENT_TIMESTAMP,platform VARCHAR(255), "
            "frequency_a VARCHAR(255), frequency_b VARCHAR(255), serial_number VARCHAR(255), mode VARCHAR(255), MicGain VARCHAR(255), "
            "Rec1SquelchLevel VARCHAR(255), Rec2SquelchLevel VARCHAR(255), "
            "NoiseBlanker1 VARCHAR(255), NoiseBlanker2 VARCHAR(255), NoiseBlankerLevel1 VARCHAR(255), NoiseBlankerLevel2 VARCHAR(255), OmOptionsInstalled VARCHAR(255), "
            "RecieverPreamp1 VARCHAR(255), RecieverPreamp2 VARCHAR(255), ReqPowerOut_Watts VARCHAR(255), PowerOut_Watts VARCHAR(100),"
    ...
    def update_settings_table(self, dict_list):
        db = mysql.connector.connect(**config_dict)
        cursor_tmp = db.cursor()
        cursor_tmp.execute("""DELETE FROM settings""")
        db.commit()
        ...
        sql = "INSERT INTO settings SET platform = '" + dict_list['platform'] + "', " \
                                                    ...
                                                    + "XITControl = '"          + dict_list['XITControl'] + "', " \
    ...
    def get_settings_table(self):
        db = mysql.connector.connect(**config_dict)
        cursor_tmp = db.cursor(dictionary=True)
        sql = "SELECT * FROM `settings` WHERE 1"
        cursor_tmp.execute(sql)
        rows = cursor_tmp.fetchall()
        row = rows[0]
        print('\n', row, '\n\n')
        for k, v in row.items():
            print('{:<22s}{:<30s}'.format(k, str(v)))
...
if __name__ == "__main__":
    ...
    #test setttings table insert
    DictElecraftCurrentSettings = {'pointer': '1', 'platform': '--',
                                            'frequency_a': '--',
                                            'frequency_b': '--',
                                            'serial_number': '123457890',
                                            'mode': 'HH',
                                            'MicGain': '24',
                                            'Rec1SquelchLevel': '--',
                                            'Rec2SquelchLevel': '--',
    ...
    #test reading select all rec from settings
    mysql_db.get_settings_table()